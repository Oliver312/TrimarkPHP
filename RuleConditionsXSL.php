...
shell_exec('find /home/ec2-user/Trimark/Inputs/temp_config/RuleConditionsXSL/ -name "*.csv" -print0 | xargs -0 rm');
...
for ($i = 2; $i < count($contents);$i++){
	  ...
	  for ($a = 2; $a < count($contents2);$a++){
		  ...
		  $str2 = str_replace(".xml","",$product);
			...
			$xsl_doc->load('/home/ec2-user/Trimark/Inputs/XSL/RuleConditions.xsl');
			...
			ob_start();
			$proc->transformToURI($xml_doc, '/home/ec2-user/Trimark/Inputs/temp_config/RuleConditionsXSL/'.$str2.'.csv');
			$outputString = ob_get_flush();
	  }  
}

// RuleConditions
shell_exec('sudo find /home/ec2-user/Trimark/Inputs/temp_config/RuleConditionsXSL/ -name "*.csv" -exec cat "{}" ";" > /home/ec2-user/Trimark/Inputs/temp_config/Merged/RuleConditions.csv');
shell_exec('/opt/Altova/MapForceServer2017/bin/mapforceserver run /home/ec2-user/Trimark/Mfx/MergeRuleConditions_Map.mfx -p=InputFileName:/home/ec2-user/Trimark/Inputs/temp_config/Merged/RuleConditions.csv -p=OutputFileName:/home/ec2-user/Trimark/Output/FinalChangedCSV/RuleConditions.csv > /home/ec2-user/Trimark/Logs/MapforceMap-log.txt 2>&1');
