...
shell_exec('find /home/ec2-user/Trimark/Inputs/temp_config/ListPropertiesXSL/ -name "*.csv" -print0 | xargs -0 rm');
...
for ($i = 2; $i < count($contents);$i++){
	  ...
	  for ($a = 2; $a < count($contents2);$a++){
		  ...
		  $str2 = str_replace(".xml","",$product);
			...
			ob_start();
			$proc->transformToURI($xml_doc, '/home/ec2-user/Trimark/Inputs/temp_config/ListPropertiesXSL/'.$str2.'.csv');
			$outputString = ob_get_flush();
	  }  
}

// ListProperties
shell_exec('sudo find /home/ec2-user/Trimark/Inputs/temp_config/ListPropertiesXSL/ -name "*.csv" -exec cat "{}" ";" > /home/ec2-user/Trimark/Inputs/temp_config/Merged/ListProperties.csv');
shell_exec('/opt/Altova/MapForceServer2017/bin/mapforceserver run /home/ec2-user/Trimark/Mfx/MergeListProperties_Map.mfx -p=InputFileName:/home/ec2-user/Trimark/Inputs/temp_config/Merged/ListProperties.csv -p=OutputFileName:/home/ec2-user/Trimark/Output/FinalChangedCSV/ListProperties.csv > /home/ec2-user/Trimark/Logs/MapforceMap-log.txt 2>&1');
